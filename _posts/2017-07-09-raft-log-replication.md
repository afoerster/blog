...
Append requests are sent asynchronously to followers. Once a majority of followers have committed the log message, the leader can commit the message. The leader keeps track of the last committed message and will communicate this to the followers. Once a follower learns that a log record was committed, it will apply the command to its own state machine.
